...
export default function MonthlyPage() {
  ...
  const fetchData = async () => {
    ...
    const { data: r } = await supabase.from('daily_records').select('*').gte('date', start).lte('date', end)
    setRecords(r || [])
  }
  ...
  const monthLabel = monthOffset === 0 ? '이번 달' : `${year}년 ${month}월`
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400">{year}년 {month}월</p>
          </div>
            ...
            <div key={m.id} className="bg-white rounded-2xl border border-gray-100 p-4 mb-3">
              <div className="flex items-center gap-3 mb-3">
                ...
                {drinkFine > 0 ? (
                  ...
                  <span className="text-green-500 text-xs">허용 범위</span>
                )}
              </div>
